...
from dpgen2.constants import (lmp_conf_name, lmp_input_name, lmp_log_name,
                              ...
                              train_task_pattern, vasp_task_pattern)
...
from mocked_ops import (MockedCollectData, MockedConfSelector,
                        MockedExplorationReport, MockedExplorationTaskGroup,
                        MockedPrepDPTrain, MockedPrepVasp, MockedRunDPTrain,
                        MockedRunLmp, MockedRunVasp, MockedSelectConfs,
                        make_mocked_init_data, make_mocked_init_models,
                        mocked_incar_template, mocked_numb_models,
...
class TestBlockCL(unittest.TestCase):
    ...
    def test(self):
        self.assertEqual(self.block_cl.keys, [
            'prep-train',
            'run-train',
            'prep-lmp',
            'run-lmp',
        ...
        for ii in range(self.numb_models):
            model = Path(block_step.outputs.artifacts["models"].local_path) / (train_task_pattern % ii) / 'model.pb'
