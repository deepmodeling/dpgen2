...
def _check_log(
    ...
    def revised_fname(ff): return Path(
        ff).name if only_check_name else Path(ff)
    tcase.assertEqual(
        lines[0].split(' '),
    ...
    for ii in range(2):
        tcase.assertEqual(
            lines[1+ii].split(' '),
            ['data', str(revised_fname(
                Path(path)/sorted(list(init_data))[ii])), 'OK']
        )
    for ii in range(2):
        tcase.assertEqual(
            lines[3+ii].split(' '),
            ['data', str(revised_fname(
                Path(path)/sorted(list(iter_data))[ii])), 'OK']
        )
    tcase.assertEqual(
        lines[5].split(' '),
...
def check_run_train_dp_output(
    ...
    if config["mode"] == "debug":
        ...
        _check_log(tcase, "log", cwd, script, init_model, init_data,
                   iter_data, only_check_name=only_check_name)
        os.chdir(cwd)
        if start != 1:
...
class TestMockedPrepDPTrain(unittest.TestCase):
    def setUp(self):
        ...
        self.expected_train_scripts = [Path(
            'task.0000/input.json'), Path('task.0001/input.json'), Path('task.0002/input.json')]
    ...
    def test(self):
        ...
        self.assertEqual([Path(ii)
                         for ii in self.expected_subdirs], op["task_paths"])
...
class TestMockedRunDPTrain(unittest.TestCase):
    ...
    def test(self):
        for ii in range(3):
            ...
            op = run.execute(ip)
            self.assertEqual(op["script"], Path(
                train_task_pattern % ii) / 'input.json')
            self.assertTrue(op["script"].is_file())
            self.assertEqual(op["model"], Path(
                train_task_pattern % ii)/'model.pb')
            self.assertEqual(op["log"], Path(train_task_pattern % ii)/'log')
            self.assertEqual(op["lcurve"], Path(
                train_task_pattern % ii)/'lcurve.out')
