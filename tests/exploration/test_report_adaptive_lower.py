...
class TestTrajsExplorationResport(unittest.TestCase):
    ...
    def test_fv(self):
        md_f = [ np.array([ 0.90, 0.10, 0.91, 0.11, 0.50, 0.53, 0.51, 0.52, 0.92 ]),
                 np.array([ 0.40, 0.20, 0.80, 0.81, 0.82, 0.21, 0.41, 0.22, 0.42 ]) ]
        md_v = [ np.array([ 0.40, 0.20, 0.21, 0.80, 0.81, 0.53, 0.22, 0.82, 0.42 ]),
        ...
        self.assertEqual(ter.candidate_ratio(), 5./18.)
        self.assertEqual(ter.accurate_ratio(), 3./18.)
    ...
    def test_f(self):
        ...
        for ii in range(2):
            for jj in picked[ii]:
                ...
                npicked += 1
        self.assertEqual(npicked, 2)
        self.assertEqual(ter.candidate_ratio(), 3./18.)
        self.assertEqual(ter.accurate_ratio(), 9./18.)
        self.assertEqual(ter.failed_ratio(), 6./18.)


    def test_v(self):
        md_v = [ np.array([ 0.90, 0.10, 0.91, 0.11, 0.50, 0.53, 0.51, 0.52, 0.92 ]),
                 np.array([ 0.40, 0.20, 0.80, 0.81, 0.82, 0.21, 0.41, 0.22, 0.42 ]) ]
        md_f = [ np.array([ 0.40, 0.20, 0.21, 0.80, 0.81, 0.53, 0.22, 0.82, 0.42 ]),
                 np.array([ 0.50, 0.90, 0.91, 0.92, 0.51, 0.52, 0.10, 0.11, 0.12 ]) ]
        expected_fail_ = [ [0, 2, 8], [2, 3, 4] ]
        expected_fail = set()
        for idx,ii in enumerate(expected_fail_):
