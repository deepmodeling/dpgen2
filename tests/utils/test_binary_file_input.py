...
class TestBinaryFileInput(unittest.TestCase):
    ...
    def test_read_file(self):
        ...
        self.assertRaisesRegex(AssertionError, 'File extension mismatch',
                               BinaryFileInput, self.task_input_path/'input_file.txt', 'jpg')

        self.assertRaisesRegex(AssertionError, 'File extension mismatch',
                               BinaryFileInput, self.task_input_path/'input_file.txt', 'tx')
        
        self.assertRaisesRegex(AssertionError, 'File extension mismatch',
                               BinaryFileInput, self.task_input_path/'input_file.txt', 't')

        self.assertRaisesRegex(AssertionError, 'File extension mismatch',
                               BinaryFileInput, self.task_input_path/'input_file', 'e')

        self.assertRaisesRegex(AssertionError, 'File extension mismatch',
                               BinaryFileInput, self.task_input_path/'input_file', 'file')
    ...
    def test_save_file(self):
        ...
        self.assertWarnsRegex(UserWarning, 'file extension mismatch!',
                              txt_with_ext.save_as_file, self.task_output_path/'output_file')
        self.assertEqual(Path(self.task_output_path/'output_file').read_text(), 'foo')
        
        self.assertWarnsRegex(UserWarning, 'file extension mismatch!',
                              txt_with_ext.save_as_file, self.task_output_path/'output_file.t')
    ...
    def test_serialization(self):
        ...
        self.assertWarnsRegex(UserWarning, 'file extension mismatch!',
                              t.save_as_file, self.task_output_path/'output_file.py')
